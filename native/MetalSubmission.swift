...
final class SubmissionCompletion: Sendable {
    ...
    var error: String? { failure.withLock { $0 } }
    init() { group.enter() }
    func finish(error: String? = nil) { failure.withLock { $0 = error }; group.leave() }
...
final class StageBindings {
    ...
    private var buffers = 0, textures = 0, samplers = 0
    init(device: any MTLDevice, descriptor: MTL4ArgumentTableDescriptor) throws {
        metal = try device.makeArgumentTable(descriptor: descriptor)
    }
    func setAddress(_ address: MTLGPUAddress, index: Int) {
        metal.setAddress(address, index: index); buffers = max(buffers, index + 1)
    }
    func setTexture(_ resource: MTLResourceID, index: Int) {
        metal.setTexture(resource, index: index); textures = max(textures, index + 1)
    }
    func setSamplerState(_ resource: MTLResourceID, index: Int) {
        metal.setSamplerState(resource, index: index); samplers = max(samplers, index + 1)
    }
    func reset() {
        for i in 0..<buffers { metal.setAddress(0, index: i) }
        for i in 0..<textures { metal.setTexture(MTLResourceID(), index: i) }
        for i in 0..<samplers { metal.setSamplerState(MTLResourceID(), index: i) }
        buffers = 0; textures = 0; samplers = 0
    }
}
...
final class CommandSlot {
    ...
    init(device: any MTLDevice) throws {
        ...
        vertex = try StageBindings(device: device, descriptor: descriptor)
        fragment = try StageBindings(device: device, descriptor: descriptor)
    }
...
final class NativeSubmission {
    ...
    func commit() {
        ...
        options.addFeedbackHandler { feedback in completion.finish(error: feedback.error?.localizedDescription) }
