...
The current build is `0.0.24-swift.15`. Swift owns resource creation, shader compilation, render pipelines, copy passes, draw encoding, render-pass lifetime and presentation. The matching native library is bundled in the jar and loaded automatically. No `metallum.nativeLibrary` JVM argument is needed; that property remains a development override only. The Java fallback has been removed.
